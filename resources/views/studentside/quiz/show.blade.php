...
@section('content')
    ...
    <div class="container">
                ...
                <form action="/active" method="post" enctype="multipart/form-data">
                    ...
                    @foreach($quiz -> question as $question)
                        ...
                        @if ($question->option_a !== null)
                            <p>
                                <input type="radio" id="{{$num}}-option_a" qid="{{$question->id}}" name="answer{{$num}}"
                                       value="option_a" {{$answers[$question->id] === 'a' ? "checked" : ""}}/>
                        ...
                        @if ($question->option_b !== null)
                            <p>
                                <input type="radio" id="{{$num}}-option_b" qid="{{$question->id}}" name="answer{{$num}}"
                                       value="option_b" {{$answers[$question->id] === 'b' ? "checked" : ""}}/>
                        ...
                        @if ($question->option_c !== null)
                            <p>
                                <input type="radio" id="{{$num}}-option_c" qid="{{$question->id}}" name="answer{{$num}}"
                                       value="option_c" {{$answers[$question->id] === 'c' ? "checked" : ""}}/>
                        ...
                        @if ($question->option_d !== null)
                            <p>
                                <input type="radio" id="{{$num}}-option_d" qid="{{$question->id}}" name="answer{{$num}}"
                                       value="option_d" {{$answers[$question->id] === 'd' ? "checked" : ""}}>
                        ...
                        @if ($question->option_e !== null)
                            <p>
                                <input type="radio" id="{{$num}}-option_e" qid="{{$question->id}}" name="answer{{$num}}"
                                       value="option_e" {{$answers[$question->id] === 'e' ? "checked" : ""}}/>
                        ...
                        <p id="{{$num}}-success" class="alert alert-success" style="display:none">Answer saved
                            successfully!</p>
        ...
        @else
            @php($num = 1)
            <p>This quiz only allows you to answer one question at a time. Some of these questions may be timed, the
                time will start ticking when you open the question. After you submit a question, you can review it here,
                but you cannot change your answer</p>
            @foreach($quiz -> question as $question)
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you wish to submit this quiz? You will not be able to change your answers after
                        submitting.</p>
